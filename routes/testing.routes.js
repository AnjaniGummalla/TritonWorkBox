...
router.get('/getlist',VerifyToken, function (req, res) {
         TestingModel.find({}, function (err, Data) {
            if (err) return res.status(500).send("There was a problem finding the data.");
            res.status(200).send(Data);
        }).populate('EmpID Project Task subTask');
});
router.get('/view/:id', VerifyToken, function (req, res) {
       TestingModel.findById(req.params.id, function (err, Data) {
          if (err) return res.status(500).send("There was a problem finding the Data.");
...
router.put('/edit/:id',VerifyToken, function (req, res) {
        TestingModel.findByIdAndUpdate(req.params.id, req.body, {new: true}, function (err, Data) {
...
router.delete('/delete/:id',VerifyToken, function (req, res) {
      TestingModel.findByIdAndRemove(req.params.id, function (err, Data) {
